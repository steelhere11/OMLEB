...
Technicians can capture GPS/time-stamped photos and collect client signatures directly in the report — replacing the external stamping app entirely. Camera capture with live overlay, gallery upload, photo labeling per equipment/step, and digital signature from the branch manager on Completado.
...
- Overlay position: compact corner badge (e.g., bottom-right) — less intrusive, smaller text
- Auto-accept after capture — photo saves immediately, tech sees it in thumbnail gallery; can delete later if bad (faster flow)
...
- Antes and despues are independent labels, NOT forced pairs — a tech can take 7 "antes" photos and only 3 "despues" for the same equipment
...
- No overlay applied to gallery uploads — only in-app camera photos get the GPS/date/time stamp
- Gallery photos without EXIF metadata are allowed — any photo can be uploaded regardless of metadata
...
- Canvas overlay rendering implementation (font size, opacity, exact formatting of corner badge)
...
- Thumbnail size and expand/lightbox behavior
...
- Gallery is a secondary input — in-app camera with overlay is the primary workflow
